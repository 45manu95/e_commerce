...
import '../../order_admin/order_admin.dart';

// ignore: must_be_immutable
class TableResi extends StatefulWidget {
  TableResi(
  ...
  @override
  State<TableResi> createState() => _TableResiState();
}

class _TableResiState extends State<TableResi> {
  ...
  int trovaIndice(
      AsyncSnapshot<List<Map<String, dynamic>>> snapshot, String id) {
    if (snapshot.hasData) {
      int index =
          snapshot.data!.indexWhere((row) => row["id"] == double.parse(id));
      return index;
    }
    return -1;
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: (widget.cfCliente == "")
          ...
          : Reso().dataResiCliente(widget.pageNumber, widget.pageSize,
                        ...
                        Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) => OrdersArea(
                                      search: snapshot.data![index]['ordine']
                                          ['numeroOrdine'],
                                      pageNumber: 0,
                                      pageSize: 10,
                                      filter: const [],
              ...
              source: WebDataTableSource(
                columns: [
                  WebDataColumn(
                    ...
                    dataCell: (value) =>
                        DataCell(Text('${value["numeroOrdine"]}')),
                  ),
                  WebDataColumn(
                    name: 'motivazione',
                    label: const Text('Motivazione'),
                        ...
                        children: [
                          GestureDetector(
                            onTap: () {
                              ...
                              Reso().deleteReso(_selectedRowKeys, context);
                              setState(() {
